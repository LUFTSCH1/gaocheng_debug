...
namespace gaocheng_debug
{
    internal static class ConstValues
    {
        ...
        public const int PathLogLines = 9;

        public const char DataIDFlag = '[';
        public const char LineEndFlag = '\n';

        public const string HashConvertFormatStr = "x2";
        ...
        public const string ProjectGeneratedFlagStr = "tested";
        ...
        public const string HashSalt = "这是个秘密";
        ...
        public static readonly string ResultTxtNotExistExceptionStr = $"{CompareResult}文件不存在{NewLine}{NewLine}导致本异常的原因可能是：{NewLine}{CompareResult}被删除{NewLine}上次测试时遇到异常，导致{CompareResult}未能生成，但用户忽略了该情况{NewLine}{NewLine}本异常不影响您继续使用该项目继续测试";
        public static readonly string PathLogExceptionStr = $"{PathLog}文件不存在、不合法或被篡改{NewLine}{NewLine}导致本异常的原因可能是：{NewLine}{PathLog}被删除{NewLine}您在test_log中手动创建了该文件夹{NewLine}您将1.5.2版本之前的项目放进了test_log{NewLine}{PathLog}被篡改{NewLine}{NewLine}解决方法：{NewLine}尝试在回收站中寻找本项目的{PathLog}文件并恢复{NewLine}删除本项目";
        public static readonly string TestProcessExceptionStr = $"cmd运行过程发生错误或被用户中断，生成{CompareResult}失败{NewLine}建议检查源程序逻辑问题 以及 每组测试数据是否合法";

        public static readonly string[] NewProjectStrSet = { " Ciallo～(∠・ω< )⌒★", " ( ｀･ω･´)ゞ", $"{NewLine}| ᐕ)⁾⁾", " ٩( ╹▿╹ )۶", " ミ(ﾉ-∀-)ﾉ", " (δωδ)」", " (灬╹ω╹灬)" };
